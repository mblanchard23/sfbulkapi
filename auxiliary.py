...
if my_settings['password']: # Looks for a password by default
  ...
  print "Password required! Please make sure you have entered one in the settings file"

def getXMLData(xmlstring,tagName):
  # improvised XML Parser :-)
  q = xmlstring.split('<'+tagName+'>')

  if len(q)> 1:
    return q[1].split('</'+tagName+'>')[0]
  else:
    return None # returns None if specified tag not found
